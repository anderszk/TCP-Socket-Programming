    ...
    try:
        with client_socket as s:
            ...
            client_socket.settimeout(
                3)  # If the client does not connect to the server within x seconds, it will time out
...
########### MÅ ENDRE NEDERST FRA login_to_server TIL login" ###############
...
def login():  # Function to login/get authorized/choose username
    ...
    with login_response as s:  # Uses the with/as function for less work(:
        if s == "loginok":
